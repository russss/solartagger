...
async def get_user(request):
    ...
    return await database.fetch_one(
        query="SELECT * FROM solartag.users WHERE id = :id",
...
async def authorize(request):
    ...
    await query(
        """INSERT INTO solartag.users(id, display_name, token)
...
async def next(request):
    ...
    try:
        if data.get("osm_id"):
            if data["action"] == "skip":
                await query(
                    """INSERT INTO solartag.results(osm_id, user_id, skip_reason)
                ...
                await query(
                    """INSERT INTO solartag.results(osm_id, user_id, module_count)
    ...
    q = """SELECT candidates.osm_id,
                ...
                HAVING solartag.int_decision(results.module_count) = false
                ORDER BY osm_id DESC
                LIMIT 1"""

    point = await database.fetch_one(q, {"user_id": request.session["user_id"]})
    surrounding = await database.fetch_all(
